                    ...
                    @forelse ($extensions as $extension)
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                @else
                                    <span class="alert alert-primary p-0">Liberação sob pagamento</span>
                                @endif
                            </td>
                            <td class="text-center">
                            ...
                            <td class="text-center">
                                @if (!$tenantExtensions->where('dashboard_extension_id', $extension->id)->first())
                                    <form action="{{ route('admin.dashboard_extension.active') }}" method="POST"
                                ...
                                @php
                                    $tenantExtension = $tenantExtensions->where('dashboard_extension_id', $extension->id)->first();
                                @endphp

                                @if ($tenantExtension && $tenantExtension->status == 1)
                                    <form action="{{ route('admin.dashboard_extension.disable', [$tenantExtension->id]) }}"
                                ...
                                @if ($tenantExtension && $tenantExtension->status == 0)
                                    <form action="{{ route('admin.dashboard_extension.enable', [$tenantExtension->id]) }}"
                                        method="POST" class="form form-inline">
                                        @csrf
                                        @method('PUT')
